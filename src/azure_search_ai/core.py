...
class AzureAISearchManager:
    ...
    def __init__(self, 
        ...
        """
        try:
            self.service_endpoint = service_endpoint or os.getenv("AZURE_AI_SEARCH_SERVICE_ENDPOINT")
            ...
            if not self.service_endpoint:
                raise ValueError("Environment variable AZURE_AI_SEARCH_SERVICE_ENDPOINT is not set.")
            if not self.index_name:
                raise ValueError("Environment variable AZURE_AI_SEARCH_INDEX_NAME is not set.")
            if not self.key:
                raise ValueError("Environment variable AZURE_SEARCH_ADMIN_KEY is not set.")

            self.search_client = SearchClient(self.service_endpoint, self.index_name, AzureKeyCredential(self.key))
            self.index_client = SearchIndexClient(endpoint=self.service_endpoint, credential=AzureKeyCredential(self.key), index_name=self.index_name)
            self.indexer_client = SearchIndexerClient(endpoint=self.service_endpoint, credential=AzureKeyCredential(self.key))

        except Exception as e:
            logger.error(f"Failed to initialize AzureAISearchManager: {e}")
            raise

    def call_azure_search_api(self, 
                              resource: str,
                              method: str, 
                              body: dict = None, 
        ...
        return response.status_code if response else None, response.json() if response else None

    def create_index(self, fields: List[Any], 
    ...
    def create_data_source_connection(self, name: str, description: str, container_name: str, type: str = "azureblob"):
        """
    ...
    def create_or_update_skillset(self, skillset_name: str, description: str, skills: List[dict]) -> None:
        """
        ...
        """
        try:
            skillset = SearchIndexerSkillset(name=skillset_name, description=description, skills=skills)
