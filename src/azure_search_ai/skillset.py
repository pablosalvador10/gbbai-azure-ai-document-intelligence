...
class IndexerManager(AzureAISearchManager):
    ...
    def create_skill(self, odata_type: str, name: str, description: str, context: str, 
                        resource_uri: Optional[str] = None, deployment_id: Optional[str] = None, 
                        inputs: Optional[Union[Dict, List[Dict]]] = None, outputs: Optional[Union[Dict, List[Dict]]] = None, 
                        uri: Optional[str] = None, batch_size: Optional[int] = None, 
        ...
        if odata_type == "EmbeddingSkill":
            ...
            if not all([azure_aoai_key, azure_aoai_resource_uri, azure_aoai_deployment_id]):
                raise ValueError("Please set AZURE_AOAI_KEY, AZURE_AOAI_RESOURCE_URI, and AZURE_AOAI_EMBEDDING_DEPLOYMENT_ID environment variables.")
    ...
    def create_indexer(self, indexer_name: str, 
        ...
        try:
            ...
            code, response = self.search_api("acs-geba", "indexers", "POST", apiVersion="2021-04-30-Preview", body=json.dumps(body))
